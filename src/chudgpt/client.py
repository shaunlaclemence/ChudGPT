...
from .schemas.quota import ChudUsageSummary
from .services.rotor import RotorService
from .utils.keys import load_secrets
from .utils.usage import UsageRules
...
class ChudMessageBuilder:
    ...
    def prompt(self, content: Any):
        self.messages_list.append(
            ChudMessage(role=ChudMessageRole.USER, content=content)
        )
        return self

    def assistant(self, content: Any):
        self.messages_list.append(
            ChudMessage(role=ChudMessageRole.ASSISTANT, content=content)
        )
        return self
...
class ChudGPT:
    ...
    def get_requests_per_day(self):
        ...
        return UsageRules.collate_usage(usages, lambda x: True)
    ...
    def get_requests_per_minute(self):
        ...
        return UsageRules.collate_usage(
            usages, lambda x: UsageRules.is_recent(x.created_at)
        )
